...
Next FrostCast episode prep | Wed 9pm (weekly) | Not Started | This is EP113 (Wed 9/23) — EP112 recorded and transcribed 2026-09-16. **Carry forward to EP113's prep sheet, Zac's call 2026-09-16:** six topics from EP112's prep sheet never got covered on air (ran long on the front half) — Mandela Catalogue movie (Spielberg/Amblin, Kister directing), Sebastian Stan's Winter Soldier pitch, Heat 2 recast/production news, The People v. Gorilla Grodd, Mandalorian and Grogu / Supergirl's streaming rebound, and the open "franchise longevity" discussion prompt. Include these on next week's list rather than letting them go stale — check whether any need a fresh news update by then before re-running them as-is.
...
Transcript delivery to Matt switched from email to Facebook Messenger | — | Done | 2026-09-16, Zac's call. Tested via `claude-in-chrome` browser automation against Zac's own logged-in Facebook session (not the Composio Facebook API connection, which can only send a message to someone with an existing Page conversation thread and can only attach a public URL, not a local file — confirmed dead end for file delivery). Sent EP111's transcript (`Spider-man cancelled BEFORE Rings of Power__ _ FrostCast Episode 111_transcript.txt`, 154 KB) as a direct file upload into Matt Khourie's personal Messenger thread; Zac confirmed it came through. Zac then had email retired entirely for this step rather than run both channels. FrostCast Transcription Workflow step 6 in root CLAUDE.md rewritten to describe the Messenger browser-upload steps in place of the old Gmail send. See `memory\topics\transcription.md` for the full technical path (why the API route fails, why the UI upload works).
